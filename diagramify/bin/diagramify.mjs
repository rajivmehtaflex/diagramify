...
const TYPES = new Set(['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'explainer-steps']);

function usage() {
  return `Usage:
  diagramify render <type> <input.json> [output.html] [--quality standard|showcase] [--repo-root path]
  diagramify compare architecture <base.json> <head.json> [output.html] [--receipt path] [--json] [--quality standard|showcase] [--repo-root path]
  diagramify deliver <type> <input.json> [output.html] [--json] [--open] [--quality standard|showcase] [--repo-root path]
  diagramify preview <type> <input.json> [output.html] [--no-open] [--quality standard|showcase] [--repo-root path]
  diagramify validate <type> <input.json> [--json] [--layout-json] [--quality standard|showcase] [--repo-root path]
  diagramify inspect <type> <input.json>
  diagramify check <output.html>
  diagramify visual-check <output.html> [--json]
  diagramify guide [scenario or question] [--json] [--lang en|zh]
  diagramify brands [name, alias, domain, or category] [--json]
  diagramify brands capture <url> [--json]
  diagramify examples
  diagramify doctor
  diagramify demo [output-directory]

Types:
  architecture, workflow, sequence, dataflow, lifecycle, explainer-steps
`;
}
...
function rendererEnv(quality, repoRoot, diagnosticJson = false) {
  return {
    ...(quality ? { DIAGRAMIFY_QUALITY_PROFILE: quality } : {}),
    ...(repoRoot ? { DIAGRAMIFY_REPO_ROOT: repoRoot } : {}),
    ...(diagnosticJson ? { DIAGRAMIFY_DIAGNOSTIC_FORMAT: 'json' } : {}),
  };
}
...
async function commandCompare(args) {
  ...
  try {
    ...
    reportCompareFailure({ json: options.json, stage: 'prepare', error: 'Architecture compare runtime is unavailable.', code: 'delta/runtime-missing', details: { reason: error.message, supportedFixes: ['install the complete Diagramify skill package'] } });
    return;
  }
  const {
  ...
  try {
    ...
    const outputDiagnostic = error.diagramifyDiagnostics?.[0];
  ...
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.diagramify-compare-'));
  } catch (error) {
...
function sourceEvidenceFromArtifact(artifact) {
  ...
  const match = html.match(/<script id="diagramify-source-evidence-data" type="application\/json">([\s\S]*?)<\/script>/);
...
async function commandDeliver(args) {
  ...
  try {
    ...
    reportDeliveryFailure({
      ...
      error: error.message,
      diagnostics: error.diagramifyDiagnostics || [diagnostic({
  ...
  let stagingDirectory;
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.diagramify-delivery-'));
  } catch (error) {
...
async function commandDoctor() {
  ...
  console.error(`\nDiagramify is not ready: ${problems.join('; ')}.`);
  process.exitCode = 1;
}
...
async function commandBrands(args) {
  ...
  if (positional[0] === 'capture') {
    if (positional.length !== 2) fail('Usage: diagramify brands capture <url> [--json]');
  ...
  if (json) {
    console.log(JSON.stringify({
      ...
      count: marks.length,
      marks,
      fallback: 'Run "diagramify brands capture <url> --json", then use the returned digest-pinned brand value.',
    }, null, 2));
    return;
  }
  if (!marks.length) {
    console.log(`No built-in brand matched "${query}". Run "diagramify brands capture <url> --json", then use the returned digest-pinned brand value.`);
    return;
  }
...
function commandDemo(args) {
  ...
  const output = path.join(outputDirectory, 'diagramify-demo.html');
  ...
  console.log('  diagramify render architecture <input.json> <output.html>');
}
...
function commandValidate(args) {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-validate-'));
