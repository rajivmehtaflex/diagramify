...
const cliPath = path.join(here, 'diagramify.mjs');
const loopbackHost = '127.0.0.1';
const defaultDebounceMs = 400;
const defaultPollMs = 800;
const defaultStopGraceMs = 3000;
const defaultStopKillMs = 750;
const diagramTypes = new Set(['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'explainer-steps']);
...
<head>
  ...
  <title>Diagramify Live Preview</title>
  <style>
...
<body data-state="checking" data-has-artifact="false">
  <header>
    <span class="brand">Diagramify Preview</span>
  ...
  <main>
    ...
    <iframe id="artifact" title="Verified Diagramify diagram"></iframe>
  </main>
  <script>
    (function () {
      'use strict';
...
export async function startPreview(options) {
  ...
  const stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.diagramify-preview-'));
    ...
    state.failure = {
      ...
      message: redactDiagnostic(
        ...
        [
          ...
          [path.resolve(here, '..'), '<diagramify-skill>'],
