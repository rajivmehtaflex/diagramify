...
explicit authored viewBox without overlap, Diagramify omits the complete legend
...
by `diagramify brands --json`, or a digest-pinned `{ "url", "sha256" }` object
returned by `diagramify brands capture <url> --json`. Known IDs and known-brand
...
strictly Diagramify judges composition. `engineering_profile` is an optional
