...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-share-card-'));
...
test('Share Card uses contain-only canonical geometry with fixed safe areas', () => {
  ...
  assert.match(html, /DIAGRAMIFY ·/);
  assert.doesNotMatch(svgBlock(html), /share-card|Share Card|DIAGRAMIFY ·/);
});
