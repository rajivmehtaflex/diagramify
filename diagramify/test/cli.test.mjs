...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-cli-'));
...
fs.writeFileSync(process.env.DIAGRAMIFY_TEST_OPEN_LOG, JSON.stringify({
...
test('cli: help lists commands and diagram types', () => {
  ...
  assert.match(result.stdout, /diagramify render <type>/);
  assert.match(result.stdout, /diagramify compare architecture <base\.json> <head\.json>/);
  assert.match(result.stdout, /diagramify deliver <type>/);
  assert.match(result.stdout, /diagramify preview <type>/);
  assert.match(result.stdout, /diagramify visual-check <output\.html>/);
  assert.match(result.stdout, /--open/);
  assert.match(result.stdout, /diagramify guide \[scenario or question\]/);
  assert.match(result.stdout, /diagramify doctor/);
  assert.match(result.stdout, /diagramify demo \[output-directory\]/);
...
test('cli: doctor reports a complete installation is ready', () => {
  ...
  assert.match(result.stdout, /\[ok\] Output path safety runtime/);
  ...
  assert.match(result.stdout, /\[ok\] workflow renderer, schema, and example/);
  assert.match(result.stdout, /\[ok\] sequence renderer, schema, and example/);
  assert.match(result.stdout, /\[ok\] dataflow renderer, schema, and example/);
  assert.match(result.stdout, /\[ok\] lifecycle renderer, schema, and example/);
  assert.match(result.stdout, /Diagramify is ready\./);
});
...
test('cli: doctor identifies an incomplete installation', () => {
  ...
  fs.copyFileSync(cli, path.join(incompleteBin, 'diagramify.mjs'));

  const result = spawnSync(process.execPath, [path.join(incompleteBin, 'diagramify.mjs'), 'doctor'], {
    cwd: incompleteRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /Diagramify is not ready: \d+ required files? missing\./);
});
...
test('cli: doctor rejects a corrupt standalone validator', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(corruptRoot, 'bin/diagramify.mjs'), 'doctor'], {
    cwd: corruptRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /Diagramify is not ready: 1 runtime check failed\./);
});
...
test('cli: examples renders from an installed skill', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(installedRoot, 'bin/diagramify.mjs'), 'examples'], {
    cwd: installedRoot,
    encoding: 'utf8',
  });
...
test('cli: guide lists all scenario recipes by diagram type', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /Diagramify scenario recipes \(11\)/);
...
test('cli: guide works from an installed skill without node_modules', () => {
  ...
  copyInstalledSkill(installedRoot);
  const installedCli = path.join(installedRoot, 'bin/diagramify.mjs');
...
test('cli: demo creates a ready-to-open diagram in a chosen directory', () => {
  ...
  assert.match(result.stdout, /diagramify render architecture/);
});
...
test('cli: demo defaults to the current directory', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.equal(fs.existsSync(path.join(workingDirectory, 'diagramify-demo.html')), true);
});
...
test('cli: visual-check returns a skipped receipt with exit 2 when Chrome is unavailable', () => {
  ...
  const result = run(['visual-check', out, '--json'], {
    env: { ...process.env, DIAGRAMIFY_CHROME: missingChrome },
  });
...
test('cli: preview runs from an installed skill without node_modules and exits cleanly', { timeout: 30000 }, async () => {
  ...
  assert.deepEqual(fs.readdirSync(path.dirname(output)).filter((name) => name.startsWith('.diagramify-preview-')), []);
});
...
test('cli: deliver preserves the previous artifact when the final check fails', () => {
  ...
  assert.deepEqual(
    fs.readdirSync(path.dirname(out)).filter((name) => name.includes('.diagramify-delivery-')),
    [],
  );
});
