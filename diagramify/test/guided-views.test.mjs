...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-guided-views-'));
...
for (const [mode, config] of Object.entries(CASES)) {
  test(`${mode}: guided views preserve base SVG geometry`, () => {
    ...
    assert.match(guided.html, /Diagramify\.guidedViews = \(function \(\)/);
    ...
    assert.match(guided.html, /--diagramify-scroll-x/);
