...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-presentation-'));
...
test('all typed renderers ship the same presentation stage contract', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Diagramify\.presentation = \(function \(\)/, mode);
...
test('presentation keyboard behavior exits in layers and remains accessible', () => {
  ...
  assert.match(html, /Diagramify\.presentation\.toggle\(\)/);
  assert.match(html, /e\.key === 'Escape' && Diagramify\.focus\.active\(\)/);
  assert.match(html, /e\.key === 'Escape' && Diagramify\.presentation\.active\(\)/);
