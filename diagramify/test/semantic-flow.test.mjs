...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-semantic-flow-'));
...
test('Semantic Flow has preset identities and motion-safe density boundaries', () => {
  ...
  assert.match(html, /@keyframes diagramify-semantic-lens-flow/);
  assert.match(html, /animation: diagramify-semantic-lens-flow 1\.35s linear 1 both/);
