...
test('start page: checked-in HTML is reproducible from canonical scenario recipes', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-start-page-'));
...
test('start page: offers five bounded bilingual starts without ingesting source content', () => {
  ...
  assert.match(html, /npx -y skills add rajivmehtaflex\/diagramify --skill diagramify --agent codex --global --copy --yes/);
  assert.match(html, /npx -y skills add rajivmehtaflex\/diagramify --skill diagramify --agent codex --copy --yes/);
  ...
  assert.match(scriptMatch[1], /diagramify:start-funnel/);
  assert.match(scriptMatch[1], /diagramify\.start\.events\.v1/);
...
test('generated artifacts omit the promotional footer and shortcut manual', () => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-start-artifacts-'));
  try {
    for (const [type, input] of Object.entries(examples)) {
      ...
      assert.doesNotMatch(html, /Built with Diagramify/, `${type}: product signature`);
...
test('viewer gives wide screens a larger canvas without forcing a subtitle row', () => {
  ...
  assert.match(template, /max-width: var\(--diagramify-reader-width, 1440px\)/);
  assert.match(template, /Diagramify\.readerLayout = \(function \(\)/);

  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-title-hierarchy-'));
  try {
