...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-gallery-'));
...
test('generated proof gallery matches its sources, receipts, and checked-in artifacts', () => {
  ...
  assert.equal(manifest.diagramifyVersion, JSON.parse(fs.readFileSync(path.join(skillRoot, 'package.json'))).version);
  ...
  assert.match(html, /href="https:\/\/github\.com\/rajivmehtaflex\/diagramify\/issues\/new\?template=showcase\.yml"[^>]+rel="noopener noreferrer"/);
