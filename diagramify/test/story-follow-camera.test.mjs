...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-story-follow-camera-'));
...
test('Story Follow frames the exact previous, current, and next authored stops through the shared camera', () => {
  ...
  assert.match(template, /Diagramify\.view\.reveal\(ids, \{/);
...
test('adaptive dwell, Still, reduced motion, hidden pages, and print keep camera motion bounded', () => {
  ...
  assert.match(template, /Diagramify\.motionGovernor && Diagramify\.motionGovernor\.capable\) return !Diagramify\.motionGovernor\.isPaused\(\)/);
...
test('pause, settle, overview, and manual camera takeover cancel Story Follow state', () => {
  ...
  assert.match(template, /function interruptCamera\(reason\)[\s\S]*?Diagramify\.guidedViews\.pause\(\)/);
