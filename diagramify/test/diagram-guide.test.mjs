...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-diagram-guide-'));
...
test('all typed renderers inherit one viewer-only Diagram Guide', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Diagramify\.guide = \(function \(\)/, mode);
    assert.match(html, /Diagram Guide — a factual command deck over existing interactions/, mode);
    assert.doesNotMatch(canonicalSvg(html), /diagram-guide|Diagramify\.guide|Explore this system/, mode);
  }
});
...
test('Diagram Guide reports compiled semantic facts and honest story availability', () => {
  ...
  assert.match(html, /return Diagramify\.guidedViews && Number\(Diagramify\.guidedViews\.count\) \|\| 0/);
...
test('Diagram Guide delegates its task rows to existing production interactions', () => {
  ...
  assert.match(html, /if \(action === 'find'\) return Diagramify\.finder\.open\(\)/);
  assert.match(html, /if \(action === 'route'\) return Diagramify\.routeProbe\.begin\(\{ focusNode: true \}\)/);
  assert.match(html, /if \(action === 'map'\) return Diagramify\.radar\.open\(\)/);
  assert.match(html, /if \(action === 'story'\) return Diagramify\.guidedViews\.play\(\)/);
  assert.match(html, /if \(action === 'present'\) return Diagramify\.presentation\.enter\(\)/);
  assert.match(html, /if \(action === 'export'\) return Diagramify\.exportMenu\.open\(\)/);
  assert.match(html, /if \(action === 'theme'\) return Diagramify\.theme\.toggle\(\)/);
  assert.match(html, /if \(action === 'reset'\) return Diagramify\.view\.reset\(\)/);
  assert.match(html, /Diagramify\.guidedViews\.pause\(\)/);
  assert.match(html, /Diagramify\.finder\.close\(\{ restoreFocus: false \}\)/);
  assert.match(html, /Diagramify\.radar\.close\(\{ restoreFocus: false \}\)/);
...
test('Diagram Guide is keyboard-first, mobile-contained, motion-safe, and embed-clean', () => {
  ...
  assert.match(html, /Diagramify\.guide\.toggle\(\)/);
