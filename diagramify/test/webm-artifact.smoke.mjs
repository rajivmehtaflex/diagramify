...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-webm-artifact-'));
const externalReachSource = process.env.DIAGRAMIFY_REACH_CARD_SOURCE
  ? path.resolve(process.env.DIAGRAMIFY_REACH_CARD_SOURCE)
  : '';
const externalReachOutput = process.env.DIAGRAMIFY_REACH_CARD_OUTPUT
  ? path.resolve(process.env.DIAGRAMIFY_REACH_CARD_OUTPUT)
  : '';
assert.equal(Boolean(externalReachSource), Boolean(externalReachOutput), 'DIAGRAMIFY_REACH_CARD_SOURCE and DIAGRAMIFY_REACH_CARD_OUTPUT must be set together');
...
function executable(candidates) {
  for (const candidate of candidates) {
    ...
    try {
      return execFileSync('sh', ['-c', `command -v "$1"`, 'diagramify-which', candidate], { encoding: 'utf8' }).trim();
    } catch (_) {
...
const ffmpeg = executable([process.env.DIAGRAMIFY_FFMPEG, 'ffmpeg']);

assert.ok(chrome, 'Chrome/Chromium is required for the WebM artifact smoke test (or set DIAGRAMIFY_CHROME)');
assert.ok(ffmpeg, 'ffmpeg is required for the WebM artifact smoke test (or set DIAGRAMIFY_FFMPEG)');
...
try {
  ...
  async function verifyResolvedLegendContract(outputs) {
    async function inspectKinds(file, expectedKinds, theme) {
      ...
      await navigateReady(url, '!!(window.Diagramify && Diagramify.semanticLens)', `legend ${theme}`);
    ...
    await navigateReady(outputs.dataflow, '!!(window.Diagramify && Diagramify.semanticLens && Diagramify.exportMenu)', 'Dataflow database legend runtime');
    const databaseRuntime = await evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      URL.createObjectURL = function (blob) {
        captured = blob.text();
        return 'blob:diagramify-dataflow-legend-smoke';
      };
      HTMLAnchorElement.prototype.click = function () {};
      try {
        await Diagramify.exportMenu.run('svg');
        var exportedText = await captured;
        var exported = new DOMParser().parseFromString(exportedText, 'image/svg+xml').documentElement;
        return {
          selected: Diagramify.semanticLens.active(),
          lensOpen: Diagramify.semanticLens.isOpen(),
    ...
    await navigateReady(outputs.custom, '!!(window.Diagramify && Diagramify.semanticLens && Diagramify.exportMenu)', 'custom legend runtime');
    const runtime = await evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      var selected = Diagramify.semanticLens.active();
      var guidedActivated = Diagramify.guidedViews.activate('main', { updateUrl: false });
      var guidedActive = Diagramify.guidedViews.active();
      var visualMatrix = [];
      for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
        if (!Diagramify.preset.apply(preset)) throw new Error('could not apply preset ' + preset);
      ...
      URL.createObjectURL = function (blob) {
        captured = blob.text();
        return 'blob:diagramify-legend-smoke';
      };
      HTMLAnchorElement.prototype.click = function () {};
      try {
        await Diagramify.exportMenu.run('svg');
    ...
    await navigateReady(embedUrl, '!!(window.Diagramify && Diagramify.semanticLens)', 'embedded legend');
    ...
    await navigateReady(outputs.hidden, '!!(window.Diagramify && Diagramify.semanticLens)', 'hidden legend');
  ...
  async function verifySemanticPassportDismissal(file) {
    await navigateReady(file, '!!(window.Diagramify && Diagramify.focus && document.querySelector("#btn-focus-clear"))', 'Semantic Passport dismissal');
    const result = await evaluate(cdp, sessionId, `(() => {
      ...
      function state() {
        return { hidden: chip.hidden, active: Diagramify.focus.active() };
      }

      Diagramify.focus.set('clients', { toggle: false, updateUrl: false });
  ...
  async function verifyArchitectureDeltaNavigator(file) {
    ...
    const exportProof = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      var explorers = frames.map(function (frame) {
        ...
        return Boolean(child && child.Diagramify && child.Diagramify.focus && child.Diagramify.routeProbe && child.document.querySelector('#btn-node-finder') && child.document.querySelector('#guided-view-play'));
      });
      var svgA = Diagramify.deltaExport.canonicalSvg();
      document.querySelector('#theme').click();
      document.querySelector('#preset').click();
      document.querySelector('.change-row').click();
      var svgB = Diagramify.deltaExport.canonicalSvg();
      var parsed = new DOMParser().parseFromString(svgB, 'image/svg+xml');
      var exportStyle = parsed.querySelector('style')?.textContent || '';
      var blob = await Diagramify.deltaExport.shareCard();
  ...
  async function captureShareCard(file, label) {
    await navigateReady(file, '!!(window.Diagramify && Diagramify.exportMenu && Diagramify.exportMenu.shareCard)', label);
    const sharePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        var blob = await Diagramify.exportMenu.shareCard();
        var bytes = new Uint8Array(await blob.arrayBuffer());
        var binary = '';
  ...
  async function captureCopiedShareCard(file, label) {
    await navigateReady(file, '!!(window.Diagramify && Diagramify.exportMenu && Diagramify.exportMenu.copyShareCard)', label);
        ...
        Object.defineProperty(navigator, 'clipboard', {
          ...
          value: {
            write: async function (items) {
              window.__diagramifyCopiedShareCard = await Promise.resolve(items[0].items['image/png']);
            }
          }
        });
        window.alert = function (message) { window.__diagramifyCopyAlert = message; };
        await Diagramify.exportMenu.copyShareCard();
        var blob = window.__diagramifyCopiedShareCard;
        if (!blob) throw new Error(window.__diagramifyCopyAlert || 'clipboard received no blob');
        var bytes = new Uint8Array(await blob.arrayBuffer());
        var binary = '';
        for (var offset = 0; offset < bytes.length; offset += 32768) {
          binary += String.fromCharCode.apply(null, bytes.subarray(offset, offset + 32768));
  ...
  async function captureRouteShareCard(file, label, sourceId, targetId, options = {}) {
    await navigateReady(file, '!!(window.Diagramify && Diagramify.routeProbe && Diagramify.exportMenu && Diagramify.exportMenu.downloadRouteShareCard)', label);
    const routePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        window.alert = function (message) { window.__diagramifyRouteAlert = message; };
        Diagramify.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
        if (!Diagramify.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
          throw new Error('route did not resolve');
        }
        var snapshot = Diagramify.routeProbe.exportSnapshot();
        if (!snapshot) throw new Error('resolved route exposed no export snapshot');
        Diagramify.exportMenu.syncRouteShare();
        ...
        var duplicateGeometryRejected = Diagramify.routeProbe.exportSnapshot() === null;
        var duplicateExportError = '';
        try { await Diagramify.exportMenu.shareCard({ variant: 'route' }); }
        ...
        var emptyGeometryRejected = Diagramify.routeProbe.exportSnapshot() === null;
        ...
        try {
          blob = await Diagramify.exportMenu.downloadRouteShareCard();
          routeReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
          ...
          if (${JSON.stringify(options.journeyInvariance === true)}) {
            async function captureJourneySource(action) {
              ...
              await Diagramify.exportMenu.shareCard({ variant: 'route' });
              routeFingerprints.push(fingerprint(await captured[captureIndex]));
            }
            await captureJourneySource(function () { Diagramify.routeProbe.selectJourneyIndex(0); });
            await captureJourneySource(function () { Diagramify.routeProbe.selectJourneyIndex(Math.floor(snapshot.nodeIds.length / 2)); });
            await captureJourneySource(function () { Diagramify.routeProbe.selectJourneyIndex(snapshot.nodeIds.length - 1); });
            Diagramify.routeProbe.showOverview({ reveal: false });
            var started = Diagramify.routeProbe.playJourney();
            ...
            Diagramify.routeProbe.pauseJourney({ preserveElapsed: false });
            await captureJourneySource(function () {});
          ...
          await Diagramify.exportMenu.run('png');
          var pngReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            canonical: document.documentElement.getAttribute('data-last-export-canonical')
          ...
          var asyncClearPromise = Diagramify.exportMenu.shareCard({ variant: 'route' });
          Diagramify.routeProbe.clear({ updateUrl: false, preserveView: true });
          ...
          await Diagramify.exportMenu.downloadRouteShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
          ...
          return {
            ...
            staleSnapshot: Diagramify.routeProbe.exportSnapshot(),
  ...
  async function verifyDynamicReducedMotionRoute(file, label, sourceId, targetId) {
    ...
    await navigateReady(file, '!!(window.Diagramify && Diagramify.routeProbe && Diagramify.exportMenu && Diagramify.motionGovernor)', label);
    ...
    const setup = await evaluate(cdp, sessionId, `(function () {
      Diagramify.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!Diagramify.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) return { resolved: false };
      Diagramify.routeProbe.showOverview({ reveal: false });
      return {
        resolved: true,
        started: Diagramify.routeProbe.playJourney(),
        playing: Diagramify.routeProbe.isJourneyPlaying(),
  ...
  async function captureRouteVisualMatrix(file, label, sourceId, targetId) {
    await navigateReady(file, '!!(window.Diagramify && Diagramify.preset && Diagramify.routeProbe && Diagramify.exportMenu)', label);
    const matrix = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      Diagramify.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!Diagramify.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
        throw new Error('route did not resolve');
      }
      var identity = JSON.stringify(Diagramify.routeProbe.exportSnapshot());
      var results = [];
      for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
        if (!Diagramify.preset.apply(preset)) throw new Error('could not apply preset ' + preset);
        for (var theme of ['dark', 'light']) {
          document.documentElement.setAttribute('data-theme', theme);
          var blob = await Diagramify.exportMenu.shareCard({ variant: 'route' });
          var bytes = new Uint8Array(await blob.arrayBuffer());
          var hash = 2166136261;
          ...
          results.push({
            ...
            identity: JSON.stringify(Diagramify.routeProbe.exportSnapshot())
          });
        }
      }
  ...
  async function captureReachShareCard(file, label, originId, direction, options = {}) {
    await navigateReady(file, '!!(window.Diagramify && Diagramify.focus && Diagramify.focus.reachabilitySnapshot && Diagramify.exportMenu && Diagramify.exportMenu.downloadReachShareCard)', label);
    const reachPayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        window.alert = function (message) { window.__diagramifyReachAlert = String(message); };
        if (!Diagramify.focus.set(${JSON.stringify(originId)}, { toggle: false, updateUrl: false })) {
          throw new Error('focus origin did not resolve');
        }
        if (!Diagramify.focus.reach(${JSON.stringify(direction)}, { toggle: false, updateUrl: false, reveal: false })) {
          throw new Error('authored reach did not resolve');
        }
        var snapshot = Diagramify.focus.reachabilitySnapshot();
        if (!snapshot) throw new Error('active authored reach exposed no export snapshot');
        Diagramify.exportMenu.syncReachShare();
        ...
        svg.appendChild(duplicateCarrier);
        var duplicateGeometryRejected = Diagramify.focus.reachabilitySnapshot() === null;
        var duplicateExportError = '';
        try { await Diagramify.exportMenu.shareCard({ variant: 'reach' }); }
        catch (error) { duplicateExportError = String(error && error.message || error); }
        duplicateCarrier.remove();
        snapshot = Diagramify.focus.reachabilitySnapshot();
        ...
        try {
          var blob = await Diagramify.exportMenu.shareCard({ variant: 'reach' });
          var reachSvgText = captured[0] ? await captured[0] : '';
          var downloadedBlob = await Diagramify.exportMenu.downloadReachShareCard();
          var reachReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            width: document.documentElement.getAttribute('data-last-export-width'),
          ...
          if (${JSON.stringify(options.matrix === true)}) {
            ...
            for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
              ...
              for (var theme of ['dark', 'light']) {
                document.documentElement.setAttribute('data-theme', theme);
                var matrixBlob = await Diagramify.exportMenu.shareCard({ variant: 'reach' });
                ...
                matrix.push({
                  ...
                  identity: JSON.stringify(Diagramify.focus.reachabilitySnapshot())
                });
              }
            }
          ...
          Diagramify.focus.clearReach({ updateUrl: false });
          Diagramify.exportMenu.syncReachShare();
          ...
          await Diagramify.exportMenu.downloadReachShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            errorFormat: document.documentElement.getAttribute('data-last-export-error-format'),
          ...
          var canonicalIndex = captured.length;
          var canonicalBlob = await Diagramify.exportMenu.shareCard();
          var canonicalSvgText = captured[canonicalIndex] ? await captured[canonicalIndex] : '';
          var canonicalSvg = parser.parseFromString(canonicalSvgText, 'image/svg+xml').documentElement;
          ...
          return {
            ...
            hiddenAfterClear: hiddenAfterClear,
            staleSnapshot: Diagramify.focus.reachabilitySnapshot(),
            staleError: staleError,
            failedReceipt: failedReceipt,
            canonicalSize: canonicalBlob.size,
  ...
  if (externalReachSource) {
    ...
    await captureReachShareCard(
      externalReachSource,
      'external-reach',
      process.env.DIAGRAMIFY_REACH_CARD_ORIGIN || 'router',
      process.env.DIAGRAMIFY_REACH_CARD_DIRECTION || 'downstream',
  ...
  await navigateReady(output, '!!(window.Diagramify && Diagramify.motion && Diagramify.motion.canRecord())', 'motion artifact');

  const payload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
    try {
      var blob = await Diagramify.motion.recordWebm({ duration: 1400, fps: 12 });
      var bytes = new Uint8Array(await blob.arrayBuffer());
      var binary = '';
      for (var offset = 0; offset < bytes.length; offset += 32768) {
        binary += String.fromCharCode.apply(null, bytes.subarray(offset, offset + 32768));
