...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-chapter-delta-preview-'));
...
test('pointer and keyboard inspect while touch and native activation still commit directly', () => {
  ...
  assert.doesNotMatch(previewRuntime, /Diagramify\.view\.|updateUrl\(|Diagramify\.focus\.|renderStoryTrail\(/);
});
...
test('latest intent, stronger owners, playback, and lifecycle cleanup remain bounded', () => {
  ...
  assert.match(html, /Diagramify\.motionGovernor\.claim\('chapter-preview'/);
  assert.match(html, /Diagramify\.motionGovernor\.release\(token\)/);
