...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-chapter-handoff-'));
...
test('handoff holds one truthful anchor then settles through one finite camera transaction', () => {
  ...
  assert.match(html, /Diagramify\.motionGovernor\.claim\('handoff'/);
  assert.match(html, /Diagramify\.motionGovernor\.release\(handoff\.ownerToken\)/);
...
test('latest intent, manual takeover, Still, reduced motion, and hidden pages cleanly settle', () => {
  ...
  assert.match(html, /Diagramify\.guidedViews\.cancelHandoff\(reason \|\| 'manual'\)/);
