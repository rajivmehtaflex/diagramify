...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-viewer-chrome-layout-'));
const chromePath = process.env.DIAGRAMIFY_CHROME ? findChrome() : null;
...
function render(mode, example) {
  ...
  execFileSync(process.execPath, [
    path.join(skillRoot, 'bin', 'diagramify.mjs'),
    'render',
    mode,
      ...
      return new Promise(function (resolve, reject) {
        ...
        function sample() {
          ...
          var current = [
            ...
            container ? getComputedStyle(container).getPropertyValue('--diagramify-nav-reserve') : ''
          ].join('|');
...
async function finalGeometry(browser, sessionId) {
  return evaluate(browser, sessionId, `(function () {
    ...
    return {
      reserve: parseFloat(getComputedStyle(container).getPropertyValue('--diagramify-nav-reserve')) || 0,
...
test('the public CLI gives all typed renderers one final Viewer contract', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(canonicalSvg(html), /nav-safe-rail|diagramify-nav-reserve|viewerChromeLayout/, mode);
    execFileSync(process.execPath, [path.join(skillRoot, 'bin', 'diagramify.mjs'), 'check', output]);
  }
});
...
test('Viewer chrome remains outside the canonical SVG export boundary', () => {
  ...
  assert.doesNotMatch(svg, /class="[^"]*\bdiagram-nav\b[^"]*"/);
  assert.doesNotMatch(svg, /nav-safe-rail|diagramify-nav-reserve|viewerChromeLayout/);
});

test('Dock Safe Rail resolves a forced Legend collision across the shared diagram viewer', {
  skip: chromePath ? false : 'Set DIAGRAMIFY_CHROME to run the real browser regression.',
}, async () => {
  ...
  try {
    ...
    for (const viewport of [
      ...
      await evaluate(browser, sessionId, `(function () {
        ...
        Diagramify.focus.set('lb', { toggle: false });
        Diagramify.radar.open();
        window.dispatchEvent(new Event('resize'));
      })()`);
      await waitForLayout(browser, sessionId);
      const receipt = await finalGeometry(browser, sessionId);
