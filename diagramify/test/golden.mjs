// Golden-file harness for the diagramify renderers. No test framework needed:
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-test-'));
...
for (const [mode, input, golden] of GOLDEN) {
  ...
  try {
    ...
    check(`${mode}: packaged ${golden}`, normalizeNewlines(fresh) === normalizeNewlines(packaged),
      `fresh render differs from diagramify/examples/${golden}; re-render the packaged examples and rebuild diagramify.zip`);
  } catch (err) {
...
check('template generator meta matches package.json version',
  template.includes(`<meta name="generator" content="diagramify ${pkg.version}">`),
