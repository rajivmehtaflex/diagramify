...
const cursorCommand = 'npx -y skills add rajivmehtaflex/diagramify --skill diagramify --agent cursor --global --copy --yes';
...
test('Cursor onboarding stays explicit, bilingual, and backed by the same Skill', () => {
  ...
  for (const surface of [english, chinese, start, landing]) {
    ...
    assert.doesNotMatch(surface, /~\/\.cursor\/skills\/diagramify/);
...
test('the zero-dependency archive works from the canonical Cursor-visible agent path', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-cursor-package-'));
  const agentSkills = path.join(tmp, '.agents', 'skills');
  try {
    fs.mkdirSync(agentSkills, { recursive: true });
    execFileSync('unzip', ['-q', path.join(repoRoot, 'diagramify.zip'), '-d', agentSkills]);
    const installed = path.join(agentSkills, 'diagramify');
    const cli = path.join(installed, 'bin', 'diagramify.mjs');
    const doctor = execFileSync(process.execPath, [cli, 'doctor'], { encoding: 'utf8' });
    assert.match(doctor, /Diagramify is ready\./);
