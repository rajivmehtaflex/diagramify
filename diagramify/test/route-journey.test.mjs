...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-route-journey-'));
...
test('motion, camera, layered Escape, mobile, print, and embed boundaries stay explicit', () => {
  assert.match(template, /Diagramify\.motionGovernor\.capable === true[\s\S]*?!Diagramify\.motionGovernor\.isPaused\(\)/);
  assert.match(template, /Diagramify\.motionGovernor\.claim\('route'/);
  assert.match(template, /reason: 'route-journey',[\s\S]*?maxScale: 1\.65,[\s\S]*?padding: 64,[\s\S]*?duration: 360/);
  assert.match(template, /Diagramify\.routeProbe\.pauseJourney\(\{ preserveElapsed: true, reason: reason \|\| 'manual' \}\)/);
  ...
  assert.match(template, /Diagramify\.routeProbe\.escape\(\{ restoreFocus: true \}\)/);
