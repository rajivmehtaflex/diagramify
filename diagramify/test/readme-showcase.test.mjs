...
test('all README languages keep the product hero and retain the verified animated proof', () => {
  for (const filename of ['README.md', 'README_EN.md', 'README_ZH.md']) {
    ...
    const titleIndex = readme.indexOf('# Diagramify');
    const proofIndex = readme.indexOf('docs/assets/archify-live-proof.gif');
    const demosIndex = Math.max(readme.indexOf('## See Diagramify in action'), readme.indexOf('## 看看 Diagramify 能做什么'));
    ...
    assert.match(readme, /https:\/\/rajivmehtaflex\.github\.io\/diagramify\/gallery\.html/);
  }
  assert.equal(
