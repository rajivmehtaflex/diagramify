...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-semantic-lens-'));
...
test('all typed renderers inherit one viewer-only Semantic Lens', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Diagramify\.semanticLens = \(function \(\)/, mode);
...
test('Semantic Lens is shareable and yields cleanly to stronger reader intent', () => {
  ...
  assert.match(html, /Diagramify\.semanticLens\.clear\(\{ updateUrl: false/);
  assert.match(html, /Diagramify\.focus\.clear\(\{ updateUrl: false, preserveView: true \}\)/);
  assert.match(html, /Diagramify\.routeProbe\.clear\(\{ updateUrl: false, restoreFocus: false \}\)/);
  assert.match(html, /Diagramify\.guidedViews\.showAll\(\{ clearFocus: false, updateUrl: false \}\)/);
  assert.match(html, /if \(action === 'lens'\) return Diagramify\.semanticLens\.open\(\)/);
  assert.match(html, /e\.key === 'l' \|\| e\.key === 'L'/);
  assert.match(html, /e\.key === 'Escape' && Diagramify\.semanticLens\.isOpen\(\)/);
  assert.match(html, /e\.key === 'Escape' && Diagramify\.semanticLens\.active\(\)/);
});
