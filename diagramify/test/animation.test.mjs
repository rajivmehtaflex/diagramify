...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-animation-'));
...
test('signal-flow preset reaches the page, SVG, and motion export surface', () => {
  ...
  assert.match(html, /Diagramify\.motion = \{ canRecord: canRecordMotion, recordWebm: recordWebm \}/);
  assert.match(html, /recorder\.requestData\(\)/);
  assert.match(html, /aria-label="Diagram view controls"/);
  assert.match(html, /Diagramify\.focus = \(function \(\)/);
  assert.match(html, /Diagramify\.view = \(function \(\)/);
...
test('editorial preset reaches every visual surface and all five typed renderers', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /data-preset-badge-editorial-plate="DIAGRAMIFY \/ PLATE 04"/, mode);
...
for (const [mode, example] of Object.entries(CASES)) {
  test(`${mode}: trace animation annotates svg, edges, and nodes`, () => {
    ...
    assert.match(svg, /aria-labelledby="diagramify-diagram-title diagramify-diagram-description"/);
    assert.match(svg, /<title id="diagramify-diagram-title">[^<]+<\/title>/);
    assert.match(svg, /<desc id="diagramify-diagram-description">[^<]+<\/desc>/);
