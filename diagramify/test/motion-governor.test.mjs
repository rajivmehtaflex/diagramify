...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'diagramify-motion-governor-'));
...
test('all renderers inherit one viewer-only Live/Still Motion Governor', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Diagramify\.motionGovernor = \(function \(\)/, mode);
...
test('reader pause is persistent, explicit, and reduced-motion aware', () => {
  ...
  assert.match(html, /var STORAGE_KEY = 'diagramify-motion'/);
  ...
  assert.match(html, /Diagramify\.guidedViews\.isPlaying\(\)[\s\S]*?Diagramify\.guidedViews\.pause\(\)/);
...
test('strong semantic intent receives the single motion budget', () => {
  ...
  assert.match(html, /animation: diagramify-route-probe-flow 1\.1s[\s\S]*?1 both/);
  assert.doesNotMatch(html, /diagramify-route-probe-flow[^;]*infinite/);
});
