...
Render `diagram_type: "dataflow"` JSON files into the standard Diagramify HTML
template.

```bash
node diagramify/renderers/dataflow/render-dataflow.mjs input.dataflow.json output.html
```

The renderer validates input against `diagramify/schemas/dataflow.schema.json`
...
`diagramify/examples/product-analytics.dataflow.json`.

The schema lives at:

```text
diagramify/schemas/dataflow.schema.json
```
...
visual-only because Diagramify has no compiled edge-kind facts in this slice. A
