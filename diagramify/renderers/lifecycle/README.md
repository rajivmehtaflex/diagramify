...
Render `diagram_type: "lifecycle"` JSON files into the standard Diagramify HTML
template.

```bash
node diagramify/renderers/lifecycle/render-lifecycle.mjs input.lifecycle.json output.html
```

The renderer validates input against `diagramify/schemas/lifecycle.schema.json`
...
`diagramify/examples/agent-run.lifecycle.json`.

The schema lives at:

```text
diagramify/schemas/lifecycle.schema.json
```
