...
Render `diagram_type: "workflow"` JSON files into the standard Diagramify HTML
template.

```bash
node diagramify/renderers/workflow/render-workflow.mjs input.workflow.json output.html
```

The renderer validates input against `diagramify/schemas/workflow.schema.json`
...
```bash
node diagramify/scripts/check-render-output.mjs output.html
```
...
`diagramify/examples/agent-tool-call.workflow.json`.

The schema lives at:

```text
diagramify/schemas/workflow.schema.json
```
