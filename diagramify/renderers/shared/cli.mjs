...
export function loadDiagram({ rendererDir, diagramType, defaultExample, argv = process.argv }) {
  ...
  const sourceEvidence = verifyRepositoryEvidence(diagramType, diagram, process.env.DIAGRAMIFY_REPO_ROOT);
...
const START_TYPES = new Set(['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'explainer-steps']);
...
export function svgRootAttrs(meta, kind) {
  ...
  const engineeringProfile = meta.engineering_profile
    ...
    : '';
  const requestedProfile = process.env.DIAGRAMIFY_QUALITY_PROFILE || meta.quality_profile;
  ...
  return `role="img" lang="${esc(resolveLocale(meta.locale))}" aria-labelledby="diagramify-diagram-title diagramify-diagram-description"${animation}${preset}${engineeringProfile} data-quality-profile="${esc(qualityProfile)}"${advisory}`;
}

// Keep the accessible name inside the SVG so it survives standalone SVG
// export and embedding. The fixed IDs are deterministic because an Diagramify
...
export function svgAccessibleText(meta, kind) {
  ...
  return `        <title id="diagramify-diagram-title">${esc(meta.title)}</title>\n        <desc id="diagramify-diagram-description">${esc(description)}</desc>`;
}
...
// canonical SVG is embedded inline outside the full Diagramify viewer.
