...
const SVG_SLOT_RE = /      <!-- DIAGRAMIFY:SVG_SLOT_START -->[\s\S]*?      <!-- DIAGRAMIFY:SVG_SLOT_END -->/;
const CARDS_SLOT_RE = /    <!-- DIAGRAMIFY:CARDS_SLOT_START -->[\s\S]*?    <!-- DIAGRAMIFY:CARDS_SLOT_END -->/;
const SUBTITLE_SLOT_RE = /^([ \t]*)<p class="subtitle">\[Subtitle description\]<\/p>[ \t]*(\r?\n)?/m;
const GUIDED_VIEWS_PLACEHOLDER = '<!-- DIAGRAMIFY:GUIDED_VIEWS_DATA -->';
const SOURCE_EVIDENCE_PLACEHOLDER = '    <!-- DIAGRAMIFY:SOURCE_EVIDENCE_DATA -->';
const I18N_PLACEHOLDER = '    <!-- DIAGRAMIFY:I18N_DATA -->';
...
export function applyTemplate(template, {
  ...
  if (!SVG_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing DIAGRAMIFY:SVG_SLOT sentinel');
  }
  if (!CARDS_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing DIAGRAMIFY:CARDS_SLOT sentinel');
  }
  ...
  const renderedSubtitle = typeof subtitle === 'string' && subtitle.trim()
    ...
    : '';
  const i18nData = `    <script id="diagramify-i18n-data" type="application/json">${i18nJson}</script>`;
  ...
  return templateWithI18n
    ...
    .replace(GUIDED_VIEWS_PLACEHOLDER, () => `<script id="diagramify-guided-views-data" type="application/json">${guidedViewsJson}</script>`)
    .replace(SOURCE_EVIDENCE_PLACEHOLDER, () => sourceEvidence
      ? `    <script id="diagramify-source-evidence-data" type="application/json">${sourceEvidenceJson}</script>`
      : '');
}
