...
const DIAGNOSTIC_MODE = process.env.DIAGRAMIFY_DIAGNOSTIC_FORMAT === 'json';
const recorded = [];
const recordedMessages = new Set();
const boundaryKey = Symbol.for('diagramify.renderer-diagnostic-boundary');
...
function normalizedDiagnostic(diagnostic) {
  const message = String(diagnostic?.message || 'Diagramify could not classify this failure.').trim();
  return {
...
export function throwDiagnosticError(message, diagnostics) {
  ...
  error.diagramifyDiagnostics = (diagnostics || []).map(normalizedDiagnostic);
  throw error;
}
...
function rendererFailure(error) {
  const attached = Array.isArray(error?.diagramifyDiagnostics)
    ? error.diagramifyDiagnostics.map(normalizedDiagnostic)
    : [];
