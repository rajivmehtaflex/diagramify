...
Render `diagram_type: "sequence"` JSON files into the standard Diagramify HTML
template.

```bash
node diagramify/renderers/sequence/render-sequence.mjs input.sequence.json output.html
```

The renderer validates input against `diagramify/schemas/sequence.schema.json`
...
`diagramify/examples/cache-miss-request.sequence.json`.

The schema lives at:

```text
diagramify/schemas/sequence.schema.json
```
